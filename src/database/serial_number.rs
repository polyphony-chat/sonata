...
impl SerialNumber {
    ...
    /// ## Errors
    ///
    /// Will error, if the [ThreadRng] fails to generate randomness. Depending
    /// on the implementation of `ThreadRng`, this method may cause a panic in
    /// these cases.
    ...
    /// Derive [Self] from 20 bytes. These bytes should be sourced from a CSPRNG
    /// or another information source with high entropy.
    ///
    /// ## Important
    ///
    /// Serial numbers with an MSB which is larger than 127 in decimal are
    /// likely not considered valid in all x509 implementations. This is
    /// because RFC 5280 is ambiguous about whether numbers which result in 21
    /// octets of ASN.1 Uint are considered valid. Sonata does not consider
    /// serial numbers with an MSB of > 127 valid for encoding, but it does
    /// consider them valid for decoding. It follows the `x509-cert` crate in
    /// this regard.
    ...
    /// ## Preamble
    ///
    /// This is just my understanding of the underlying problem. Feel free to
    ...
    /// ```
    ///
    /// This means that the first octet of a 20-octet array may not be larger
    /// than 128 in decimal, if the resulting [SerialNumber] is to be a valid
    /// x509 serial number regardless of how you may interpret the spec.
    ///
    /// ## The (hacky) solution
    ///
    /// To adjust for this, we simply take the modulo 128 of the first octet in
    /// the array. This way, we have a lossy projection from the CSPRNG
    /// generated first-maybe-valid-octet to a definitely-valid first octet.
    ///
    /// ## Is this cryptographically safe?
    ///
    /// I don't know. :3
    ///
    /// But I suspect that it is sufficient for this purpose, because serial
    /// numbers are not meant to be cryptographically safe on their own; they
    /// should likely just be random *enough*. In my head, the worst case is
    /// that instead of 160 bits of entropy, there will still be 159
